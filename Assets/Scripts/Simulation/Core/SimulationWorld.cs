...
{
    /// <summary>
    /// World host. Daily path: DailySimulation → calendar → events.
    ...
    public class SimulationWorld : MonoBehaviour
    {
        ...
        /// <summary>
        /// One simulation day:
        /// Season → Weather → Resources → Population → Society → Calendar → Events.
        /// </summary>
        ...
        public void FastForwardYears(int years)
        {
            ...
            autoRun = false;
            var result = FastForwardSystem.FastForwardYears(State, Races, Config, Influence, years, seed);
            State = result.State;
            Influence.Bind(State);
            SeasonSystem.SyncFromCalendar(State);
        ...
        public string RunConsistencyTestOneYear()
        {
            ...
            var report = FastForwardConsistencyTest.RunOneYear(State, Races, Config, Influence, seed);
        ...
        public string BuildStatusReport()
        {
            ...
            sb.AppendLine($"季节: {SeasonSystem.DisplayName(State.CurrentSeason)} ({State.CurrentSeason}) 季内第 {State.DayInSeason} 日 进度 {State.SeasonProgress:0%}  Index={State.SeasonIndex}");
            ...
            foreach (var r in State.Regions)
            {
                ...
                sb.AppendLine($"人口 {r.Population:0} (Δ {r.PopulationDelta:+0.0;-0.0}) | 承载力 {r.LastCarryingCapacity:0} | 稳定 {r.Stability:0.00} | 教育 {r.Education:0.00} | 信仰 {r.FaithLevel:0.00} | 疫病 {r.DiseasePressure:0.00}");
