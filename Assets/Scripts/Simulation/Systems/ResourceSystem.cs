...
namespace DivineWorld.Simulation.Systems
{
    ...
    public static class ResourceSystem
    {
        ...
        public static float CalculateWaterProduction(RegionState region, SeasonId season, SimulationConfig config, float env)
        {
        ...
        public static float CalculateTimberProduction(RegionState region, RaceDefinition race, SimulationConfig config, float labor, float tech)
        {
            if (race.PrefersSea)
            {
        ...
        public static float CalculateOreProduction(RegionState region, RaceDefinition race, SimulationConfig config, float labor, float tech)
        {
            if (race.PrefersSea)
            {
            ...
            float harvest)
        {
            switch (id)
            {
                ...
                case ResourceId.Water:
                    return CalculateWaterProduction(region, season, config, env);
                case ResourceId.Timber:
                    return CalculateTimberProduction(region, race, config, labor, tech);
                case ResourceId.Ore:
                    return CalculateOreProduction(region, race, config, labor, tech);
                case ResourceId.Faith:
                    return region.Population * config.FaithYieldPerCapita * region.FaithLevel * race.FaithTendency;
                case ResourceId.Knowledge:
                    return region.Population * config.KnowledgeYieldPerCapita * region.Education * race.KnowledgeTendency;
                case ResourceId.Magic:
                    return region.Population * config.MagicYieldPerCapita * race.MagicAffinity
                           * (race.PrefersSea ? config.SeaMagicAffinityBonus : config.LandMagicAffinityBonus);
                default:
                    return 0f;
            }
